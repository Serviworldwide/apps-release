...
# Pushes: payterminal-app-staging-release.apk
# Usage: ./push-payterminal-staging.sh <commit message>   (message may be unquoted / multi-word)

MSG="$*"
if [ -z "$MSG" ]; then
  echo "Usage: ./push-payterminal-staging.sh <commit message>"
  exit 1
fi

FILES=(payterminal-app-staging-release.apk)
