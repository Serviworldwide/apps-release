...
# Pushes: payterminal-app-prod-release.apk
# Usage: ./push-payterminal-prod.sh <commit message>   (message may be unquoted / multi-word)

MSG="$*"
if [ -z "$MSG" ]; then
  echo "Usage: ./push-payterminal-prod.sh <commit message>"
  exit 1
fi

FILES=(payterminal-app-prod-release.apk)
