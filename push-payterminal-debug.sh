...
# Pushes: payterminal-app-debug.apk
# Usage: ./push-payterminal-debug.sh <commit message>   (message may be unquoted / multi-word)

MSG="$*"
if [ -z "$MSG" ]; then
  echo "Usage: ./push-payterminal-debug.sh <commit message>"
  exit 1
fi

FILES=(payterminal-app-debug.apk)
